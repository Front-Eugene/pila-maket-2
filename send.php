...
$message = [
    'api_token' => 'ZCWJcIuKQY1ZGEX06y5pLPTYsI1zeHqzVPpTjawprVKcSu3jsc4B9U6Bvr6n', //'api_ключ, указан здесь /settings/integrations',
    'order' => [
        ...
        //'postcode' => '220050', //Индекс
        //'region' => 'Минская', //Область
        //'district' => 'Минский', //Район
        //'city' => 'Минск', //Город
        //'street' => 'пер. Стиковский', //Улица
        //'house' => '4', //Дом
        //'unit' => '2', //Корпус
        //'entrance' => '3', //Подъезд
        //'apartment' => '67', //Квартира
        //'floor' => '5', //Этаж

        'source_id' => 184, //Источник, указать id источника в CRM
        //'manager_id' => 6, //Менеджер, указать id менеджера в CRM
        //'task_date' => '2019-01-01', //Напоминание - дата, указывать в фомате ГГГГ-ММ-ДД
        //'task_time' => '09:00', //Напоминание - время, указывать в фомате ЧЧ:ММ
        //'addition_id' => 'MP364920124532', //Дополнительный идентификатор клиента
        'delivery_type_id' => 0, //Доставка - тип, (1 - курьер, 2 - самовывоз, 3 - белпочта)
        'sender_id' => 1, //Доставка - отправитель, указать id отправителя в CRM
        //'delivery_date' => '2019-12-31', //Доставка - дата, указывать в фомате ГГГГ-ММ-ДД
        //'delivery_time_from' => '19:00', //Напоминание - время с, указывать в фомате ЧЧ:ММ
        //'delivery_time_to' => '22:00', //Напоминание - время до, указывать в фомате ЧЧ:ММ

        'products' => [
            //Товар 1
            [
                'offer_id' => 1907, //id товара в CRM
                'price' => null, //цена товара в заказе
                'quantity' => 1, //кол-во товара в заказе
                //'comment' => '' //комментарий к товару
            ],
            //Товар 2, не обязательно
            /*[
                'offer_id' => 3, //id товара в CRM
                'price' => 0, //цена товара в заказе
                'quantity' => 1, //кол-во товара в заказе
                'comment' => 'Подарок', //комментарий к товару
            ]*/
        ],

        //'customFields' => [
        //    21 => $_POST['i_recipient'], //Доп. поле, где 21 - id доп. поля в CRM
        //    83 => $_POST['i_age'], //Доп. поле, где 83 - id доп. поля в CRM
        //]

    ],
];

$curl2 = curl_init('https://supersale.pro/api/api/orders');
curl_setopt($curl2, CURLOPT_RETURNTRANSFER, true);
curl_setopt($curl2, CURLOPT_POST, true);
curl_setopt($curl2, CURLOPT_POSTFIELDS, json_encode($message));
curl_setopt($curl2, CURLOPT_HTTPHEADER, [
  'Content-Type: application/json',
  'X-Forwarded-For: ' . $_POST['user_ip'], // Установка IP пользователя в заголовок
  'X-Disable-RateLimit: true' // Отключение защиты от спама
]);

if (!empty($_POST["phone"])) {
  $result = json_decode(curl_exec($curl2), true);
  $httpCode = curl_getinfo($curl2, CURLINFO_HTTP_CODE);
}
curl_close($curl2);


if ($httpCode == 429) { // Проверка кода состояния HTTP 429 Too Many Requests
  $string = "Превышен лимит запросов. Пожалуйста, попробуйте снова позже.";
} elseif (isset($result['number'])) {
  $orderNumber = $result['number'];
  $string = "Ваш заказ № $orderNumber успешно создан!<br>Менеджер свяжется с вами в ближайшее время";
} else {
  $string = "Произошла ошибка при обработке вашего запроса.";
}?>
    ...
    <div class="screen">
        ...
        <div class="content centered with-centered-items">
            ...
            ">
                    ...
                    <h1 class="inline">Спасибо за заявку!</h1>
                    <p>Мы закрепим за вашим номером подарок. Мы свяжемся с Вами в ближайшее время!</p>
                    <a class="btn" href="#" onclick="javascript:history.back();">Вернуться назад</a>
                </div>
            </div>
        </div>
